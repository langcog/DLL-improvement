...
Below are shown the parameters for items from the DLL Level 1 long supplement: en_d = English easiness, sp_d = Spanish easiness), ordered by the most to least discrepant (difficulty difference squared); (_a1 columns show item discriminations (slopes), and sp_en_d_diff simply shows difference in Spanish and English easiness parameter).
Clearly some of these items have quite different difficulty levels in English and Spanish, and we should try to find items that are more equivalent.
...
Even swapping only 15 items reduced the total SSE by more than 50%.
