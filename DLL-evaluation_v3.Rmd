...
Using data from `r nrow(subset(d_demo_en, age<19))` English-speaking children 12-18 month of age from Wordbank, we test how well sumscores from the DLL-ES1 short English form + CDI:WG short form predict children's  production scores from the long form (LF) CDI (WG/WS).
...
p1 <- old_sp %>% ggplot(aes(x=production, y=DLL1sum)) + # , group=sex, color=sex
  geom_point(alpha=.3) +  
  xlab("CDI:WS LF Production Score") + ylab("DLL-ES2 Short Score") + 
