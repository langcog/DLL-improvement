...
```{r dll-error, echo=F, include=F}
DLL_rel = psych::ICC(young_en[,c("production","DLLsum")]) # ICC3 .66
WG_rel = psych::ICC(young_en[,c("production","WGshort")]) # ICC3 .43

dll1_en_rmse <- young_en %>% mutate(DLL_rmse = sqrt((production - DLLsum)^2),
                    WGshort_rmse = sqrt((production - WGshort)^2)) %>% 
  summarise(DLL_rmse = mean(DLL_rmse), WGshort_rmse = mean(WGshort_rmse))
```

The reliability (ICC3) of the DLL + WG short form vs. the full CDI:WG score is `r round(DLL_rel$results$ICC[3], 2)`, higher than the reliability of just the WG short form (ICC3 = `r round(WG_rel$results$ICC[3], 2)`).
...
```{r dll2-error, echo=F, include=F}
DLL2_rel = psych::ICC(old_en[,c("production","DLL2sum")]) # ICC3 .44
WS_rel = psych::ICC(old_en[,c("production","WSshort")]) # ICC3 .25

dll2_en_rmse <- old_en %>% mutate(DLL_rmse = sqrt((production - DLL2sum)^2),
                    WSshort_rmse = sqrt((production - WSshort)^2)) %>% 
  summarise(DLL_rmse = mean(DLL_rmse), WSshort_rmse = mean(WSshort_rmse))
```

The reliability (ICC3) of the DLL + WS short form vs. the full CDI:WS score is `r round(DLL2_rel$results$ICC[3], 2)`, higher than the reliability of just the WS short form (ICC3 = `r round(WS_rel$results$ICC[3], 2)`).
...
```{r dll-sp-error, echo=F, include=F}
DLL_sp_rel = psych::ICC(young_sp[,c("production","DLL1sum")]) 
WG_sp_rel = psych::ICC(young_sp[,c("production","WGshort")]) 

dll1_sp_rmse <- young_sp %>% mutate(DLL_rmse = sqrt((production - DLL1sum)^2),
                    WGshort_rmse = sqrt((production - WGshort)^2)) %>% 
  summarise(DLL_rmse = mean(DLL_rmse), WGshort_rmse = mean(WGshort_rmse))
```

The reliability (ICC3) of the DLL + WG short form vs. the full CDI:WG score is `r round(DLL_sp_rel$results$ICC[3], 2)`, higher than the reliability of just the WG short form (ICC3 = `r round(WG_sp_rel$results$ICC[3], 2)`).
...
```{r dll2-sp-error, echo=F, include=F}
DLL2_sp_rel = psych::ICC(old_sp[,c("production","DLL2sum")]) 
WS_sp_rel = psych::ICC(old_sp[,c("production","WSshort")]) 

dll2_sp_rmse <- old_sp %>% mutate(DLL_rmse = sqrt((production - DLL2sum)^2),
                    WSshort_rmse = sqrt((production - WSshort)^2)) %>% 
  summarise(DLL_rmse = mean(DLL_rmse), WSshort_rmse = mean(WSshort_rmse))
```

The reliability (ICC3) of the DLL + WS short form vs. the full CDI:WS score is `r round(DLL2_sp_rel$results$ICC[3], 2)`, higher than the reliability of just the WS short form (ICC3 = `r round(WS_sp_rel$results$ICC[3], 2)`).
